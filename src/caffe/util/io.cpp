...
bool ReadBoxDataToDatum(const string& filename, const string& annoname,
  ...
  if (cv_img.data) {
    if (encoding.size()) {
      ...
      //read txt anno data
      ParseTxtToDatum(annoname, label_map, ori_w, ori_h, datum);
      return true;
    }
    CVMatToDatum(cv_img, datum);
    // read xml anno data
...
void ParseTxtToDatum(const string& annoname, const map<string, int>& label_map, int ori_w, int ori_h, Datum* datum) 
{
  FILE* fp = fopen(annoname.c_str(), "r");
  assert(fp != NULL);
  ...
  {
     datum->add_float_data(float(id));
     datum->add_float_data(float(0));
     datum->add_float_data(x);
     datum->add_float_data(y);
     datum->add_float_data(w);
     datum->add_float_data(h);
  }
  fclose(fp);
}
